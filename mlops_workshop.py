# -*- coding: utf-8 -*-
"""MLOps_Workshop.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1eodbLdIMqeiVn-WpEPlyVYB5-EaLLQel
"""
...
x=np.linspace(1,100,30,endpoint=False) #30 point datasets between 1 to 100
print(x)
...
z=m*x
print(type(z))
z=z+b #y=wx+b regression line
plt.plot(x,z,color='red')

def cost(p):
  c=0
  for i in range(30):
    c+=(y[i]-p[i])*(y[i]-p[i]) #mean square error
...
def update(m,b,lr,lr2):
  ...
  grad=np.sum(np.dot(-1*(x.T),y-(m*x + b))) #for slope
  gradb=np.sum(-1*(y-(m*x + b))) #for y coords
...
m=-2 #providing a completely new slope for train and update
b=-200
z=m*x
z=z+b
for i in range(600):
